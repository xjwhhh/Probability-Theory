...
def estimate():
    ...
    for j in range(number_experiments):
        N = number_trans(upper_bound)#火车头数量
        evidence = train_seen(N)#观测到的火车头号

        hypo1 = MLE_estimation(evidence)#极大似然估计
        hypo2 = MSE_estimation(evidence)#最小偏方差估计
        hypo3 = ME_estimation(evidence)#无偏估计
