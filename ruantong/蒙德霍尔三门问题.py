...
def MontyHall(Dselect, Dchange):
    Dcar = random.randint(1, 3)#哪一扇门有汽车
...
if __name__ == '__main__':
    ...
    win = 0
    for i in range(n):
        Dselect = random.randint(1, 3)#参赛者选中的门,用于生成一个指定范围内的整数。其中参数a是下限，参数b是上限，生成的随机数n: a <= n <= b
        Dchange = random.randint(0, 1)#参赛者变不变，0不改变，1改变
